 ...
 * @author Brunoli
 */
public class InsertThreadPersistencia implements Runnable {

    private PruebaInnodbJpaController jpa;

    public InsertThreadPersistencia() {
        jpa = new PruebaInnodbJpaController();
    }

    @Override
    public void run() {
        PruebaInnodb ipdr = new PruebaInnodb();
        cargarIPDR(ipdr);
        PruebaInnodbPK pk;
        int aux = 0;
        int aux2 = 0;
        while (true) {
            ...
            aux2++;
            if (aux > 500) {
                System.out.println(aux2 + " INSERT EN LA TABLA INNODB");
                aux = 0;
            }
        }
    }

    private void cargarIPDR(PruebaInnodb ipdr) {
